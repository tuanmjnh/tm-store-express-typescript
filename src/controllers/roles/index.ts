...
module.exports.insert = async function(req, res, next) {
  try {
    ...
    data.save((e, rs) => {
      if (e) return res.status(500).send(e)
      // Push logs
      logs.push(req, { user_id: verify._id, collection: 'roles', collection_id: rs._id, method: 'insert' })
...
module.exports.lock = async function(req, res, next) {
  try {
    ...
    for await (let _id of req.body._id) {
      ...
      if (x) {
        ...
        if (_x.nModified) {
          rs.success.push(_id)
          // Push logs
          logs.push(req, { user_id: verify._id, collection: 'roles', collection_id: _id, method: x.flag === 1 ? 'lock' : 'unlock' })
    ...
    //     const x = await Model.findById(e)
    //     if (x) {
    //       Model.updateOne({ _id: req.params.id }, { $set: { flag: req.body.flag } }, (e, rs) => {
    //         if (e) return res.status(500).send(e)
    //         if (!rs) return res.status(404).send('no_exist')
    //         return res.status(202).json(rs)
    //       })
  ...
  try {
    ...
    if (mongoose.Types.ObjectId.isValid(req.params._id)) {
      Model.deleteOne({ _id: req.params._id }, (e, rs) => {
        if (e) return res.status(500).send(e)
        // Push logs
        logs.push(req, { user_id: verify._id, collection: 'roles', collection_id: req.params._id, method: 'delete' })
