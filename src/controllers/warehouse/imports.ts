...
module.exports.finds = async function(req, res, next) {
  try {
    ...
    const rs = await ModelProducts.find({ code: { $in: req.body } }, '_id code title quantity price price_discount price_import price_unit unit')
...
module.exports.imports = async function(req, res, next) {
  ...
  try {
    ...
    const total = new Model({
      ...
      total_price: req.body.sum('price_import'),
      total_quantity: req.body.sum('quantity_import'),
      created_at: new Date(),
    ...
    rs.data = totalSave
    // Push logs import
    logs.push(req, { user_id: verify._id, collection: 'product_imports', collection_id: totalSave._id, method: 'insert' })
    // Loop item
    for await (const e of req.body) {
      ...
      const items = new ModelItems({
        import: totalSave._id,
        product: e._id,
        price: e.price_import,
        quantity: e.quantity_import
      })
      // items.validate()
      const itemsSave = await items.save()
      if (!itemsSave) throw new Error('import item')
      ModelProducts.updateOne({ _id: e._id }, {
        $set: { price_import: parseInt(e.price_import) },
        $inc: { quantity: parseInt(e.quantity_import) }
      }).exec()
    }
    // commit
