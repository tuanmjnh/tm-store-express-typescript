...
module.exports.insert = async function(req, res, next) {
  try {
    ...
    data.save((e, rs) => {
      if (e) return res.status(500).send(e)
      // Push logs
      logs.push(req, { user_id: verify._id, collection: 'routes', collection_id: rs._id, method: 'insert' })
  ...
  try {
    ...
    if (mongoose.Types.ObjectId.isValid(req.body._id)) {
      ...
      Model.updateOne({ _id: req.body._id }, {
        ...
        if (e) return res.status(500).send(e)
        // Push logs
        logs.push(req, { user_id: verify._id, collection: 'routes', collection_id: req.body._id, method: 'update' })
    ...
    for await (let _id of req.body._id) {
      ...
      if (x) {
        ...
        if (_x.nModified) {
          rs.success.push(_id)
          // Push logs
          logs.push(req, { user_id: verify._id, collection: 'routes', collection_id: _id, method: x.flag === 1 ? 'lock' : 'unlock' })
  ...
  try {
    ...
    if (mongoose.Types.ObjectId.isValid(req.params._id)) {
      Model.deleteOne({ _id: req.params._id }, (e, rs) => {
        if (e) return res.status(500).send(e)
        // Push logs
        logs.push(req, { user_id: verify._id, collection: 'routes', collection_id: req.params._id, method: 'delete' })
